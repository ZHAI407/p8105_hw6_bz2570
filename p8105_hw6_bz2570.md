...
Among the cities with the lowest ORs, **New York, NY** stand out, with
estimated ORs clearly below 0.5, indicating that homicides involving
male victims have lower odds of being solved compared to those involving
female victims. In contrast, several cities display relatively high ORs.
For example, **Albuquerque, NM**, **Stockton, CA** and **Fresno, CA**
show OR estimates above 1, suggesting a higher likelihood of solving
cases involving male victims. However, these higher ORs often come with
wide confidence intervals, indicating substantial uncertainty in the
estimates.
...
Stepwise Regression Results for Birthweight

``` r
stepwise_formula = formula(stepwise_model)
```

**Modeling process:** This model based on a data-driven model-building
process. Using stepwise regression (both directions) to select the most
relevant predictors for birthweight from all available variables.
...
![](p8105_hw6_bz2570_files/figure-gfm/unnamed-chunk-11-1.png)<!-- -->
...
![](p8105_hw6_bz2570_files/figure-gfm/unnamed-chunk-13-1.png)<!-- -->
...
| model_name        | mean_rmse | sd_rmse |
|:------------------|----------:|--------:|
| Stepwise Model    |   274.023 |   9.026 |
| Interaction Model |   289.473 |  10.241 |
| Linear Model      |   333.725 |  16.386 |

Among the three models, the stepwise regression have the best predictive
performance with a cross-validated RMSE of 274.023.
